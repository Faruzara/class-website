...
import { scheduleItemTone } from "@/components/schedule/schedule-colors";
              ...
              {DAYS.map((day) => {
                ...
                return (
                  <div key={day} className="grid min-h-24 border-b border-surface-border last:border-0" style={{ gridTemplateColumns: `100px repeat(${maxPeriod}, minmax(70px, 1fr))` }}>
                    ...
                    {dayItems.map((item) => (
                      <div key={item.id} className={`z-10 m-1.5 flex flex-col justify-center rounded-lg border px-3 py-2 ${scheduleItemTone(item)}`} style={{ gridColumn: `${item.start_period + 1} / ${item.end_period + 2}`, gridRow: 1 }}>
