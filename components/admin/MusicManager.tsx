...
import { ArrowDown, ArrowUp, CheckCircle2, ChevronDown, ChevronUp, Eye, EyeOff, ExternalLink, HelpCircle, Loader2, Plus, RefreshCw, Save, Search, Trash2, X, XCircle } from "lucide-react";
...
function StatusBadge({ status, configured, checkedAt }: { status: string; configured: boolean; checkedAt: string | null }) {
  const states: Record<string, { icon: ReactNode; label: string; className: string }> = {
    ...
    error: { icon: <XCircle size={13} />, label: "Pencarian perlu OAuth", className: "border-amber-200 bg-amber-50 text-amber-700" },
...
export default function MusicManager({ initial }: { initial: InitialState }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm"><header className="border-b border-gray-100 px-5 py-4"><h2 className="font-display text-base font-semibold text-gray-900">Tambah lagu</h2></header><div className="space-y-3 px-5 py-4">
      <div className="flex gap-2"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void search(); }} disabled={status === "expired" || status === "error"} placeholder="Cari lagu atau artis di SoundCloud" className="min-w-0 flex-1 rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-gray-400 disabled:bg-gray-50" /><button onClick={search} disabled={busy === "search" || status === "expired" || status === "error"} className="grid size-10 shrink-0 place-items-center rounded-xl bg-gray-900 text-white disabled:opacity-40">{busy === "search" ? <Loader2 className="animate-spin" size={15} /> : <Search size={15} />}</button></div>
      {status === "expired" ? <p className="text-xs text-rose-600">Pencarian tidak tersedia sampai Client ID diperbarui.</p> : status === "error" ? <p className="text-xs text-amber-700">Gunakan link SoundCloud untuk menambahkan lagu tanpa pencarian.</p> : null}
      {results.length ? <div className="overflow-hidden rounded-xl border border-gray-100 bg-gray-50"><button onClick={() => setResultsOpen((value) => !value)} className="flex w-full items-center justify-between px-4 py-2.5 text-xs font-medium text-gray-500"><span>{results.length} hasil pencarian</span>{resultsOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>{resultsOpen ? <div className="divide-y divide-gray-100 border-t border-gray-100">{results.map((track) => <div key={track.soundcloud_url} className="flex items-center gap-3 px-4 py-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-gray-800">{track.title}</p><p className="truncate text-xs text-gray-500">{track.artist}</p></div><button onClick={() => add(track)} disabled={Boolean(busy)} className="grid size-8 shrink-0 place-items-center rounded-lg border border-gray-200 bg-white disabled:opacity-40" aria-label={`Tambahkan ${track.title}`}><Plus size={14} /></button></div>)}</div> : null}</div> : null}
