...
export default function MusicManager({ initial }: { initial: InitialState }) {
  ...
  async function saveClientId() {
    ...
    } catch (error) { setConfigured(true); setCheckedStatus("expired"); setMessage(error instanceof Error ? error.message : "Gagal menyimpan."); }
    finally { setBusy(""); }
  }
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm"><header className="border-b border-gray-100 px-5 py-4"><h2 className="font-display text-base font-semibold text-gray-900">Tambah lagu</h2></header><div className="space-y-3 px-5 py-4">
      <div className="flex gap-2"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void search(); }} disabled={status === "expired"} placeholder="Cari lagu atau artis di SoundCloud" className="min-w-0 flex-1 rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-gray-400 disabled:bg-gray-50" /><button onClick={search} disabled={busy === "search" || status === "expired"} className="grid size-10 shrink-0 place-items-center rounded-xl bg-gray-900 text-white disabled:opacity-40">{busy === "search" ? <Loader2 className="animate-spin" size={15} /> : <Search size={15} />}</button></div>
      {status === "expired" ? <p className="text-xs text-rose-600">Pencarian tidak tersedia sampai Client ID diperbarui.</p> : null}
